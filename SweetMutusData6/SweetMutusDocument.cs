...
namespace Aldentea.SweetMutus.Net6.Data
{
	...
	public class SweetMutusDocument : DocumentWithOperationHistory
	{
		...
		public void ImportSongs(IEnumerable<ISong> songs, string category = "")
		{
		...
		/// <returns></returns>
		public XDocument GenerateXml(string destination_directory, string exported_songs_root = "")
		{
			XDocument xdoc = new XDocument(
				new XElement(ROOT_ELEMENT_NAME,
					new XAttribute(VERSION_ATTERIBUTE, "3.0"),
					GenerateSweetMutusElement(destination_directory, exported_songs_root)
				)
			);
			return xdoc;
		}
		...
		/// <returns></returns>
		protected virtual XElement GenerateSweetMutusElement(string destination_directory, string exported_songs_root = "")
		{
		...
		/// そうでなければnullを与えます．</param>
		/// <returns></returns>
		public XDocument GenerateMtuXml(string destination_directory, string exported_songs_root = "")
		{
			XDocument xdoc = new XDocument(
				new XElement(ROOT_ELEMENT_NAME,
					new XAttribute(VERSION_ATTERIBUTE, "3.0"),
					Questions.GenerateQuestionsElement(),
					Questions.GenerateSongsElement(destination_directory, exported_songs_root)
				)
			);
			return xdoc;
		}
		...
		public async Task SaveExport(string destination, string songs_root)
		{
			...
			switch (Path.GetExtension(destination))
			{
				case ".mtq":
					...
					{
						await GenerateMtuXml(Path.GetDirectoryName(destination), songs_root).WriteToAsync(writer, token);
					}
					break;
				default:
					using (XmlWriter writer = XmlWriter.Create(destination, this.WriterSettings))
					{
						await GenerateXml(Path.GetDirectoryName(destination), songs_root).WriteToAsync(writer, token);
					}
					break;
			}
		}
		...
		protected async override Task<bool> LoadDocument(string fileName)
		{
			...
			using (var reader = XmlReader.Create(fileName, new XmlReaderSettings { Async = true }))
			{
				...
				if (root != null && root.Name == ROOT_ELEMENT_NAME)
				{
					...
					if (version.HasValue)
					{
						if (version >= 3.0M)
						{
							var result = TryLoadSweetMutusDocument(root, Path.GetDirectoryName(fileName) ?? string.Empty);
              if (result.HasValue)
							{
								return result.Value;
							}
						...
						else if (version >= 2.0M)
						{
							...
							if (songs != null && Confirm("mutus2のファイルを読み込みます．保存するときにSweetMutusの形式に変換することになります(情報の一部が失われることがあります)．\n"
								+ "処理を続行しますか？"))
							{
								NowLoading = true;
								try
		...
		{
			var element = sweet.Element(SweetQuestionsCollection.ELEMENT_NAME);
			if (element != null)
			{
				this.Questions.LoadElement(element, fileDirectory);
			}
		}
		...
		async Task<bool> SaveSmtDocument(string destination)
		{
			...
			using (XmlWriter writer = XmlWriter.Create(destination, this.WriterSettings))
			{
				// ☆Path.GetDirectoryNameは"C:\"を与えるとnullを返すけど、その対応が必要かも！
				await GenerateXml(Path.GetDirectoryName(destination)).WriteToAsync(writer, token);
			}
			// 基本的にtrueを返せばよろしい．
		...
		async Task<bool> SaveMtqDocument(string destination)
		{
			...
			using (XmlWriter writer = XmlWriter.Create(destination, this.WriterSettings))
			{
				await GenerateMtuXml(Path.GetDirectoryName(destination)).WriteToAsync(writer, token);
			}
			return true;
		}
