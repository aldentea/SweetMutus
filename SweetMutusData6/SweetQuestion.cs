...
//			return ret;
		...
		public static SweetQuestion Generate(XElement questionElement, string songsRoot = "")
		{
			...
			question.Title = (string?)questionElement.Element(TITLE_ELEMENT) ?? String.Empty;
			question.Artist = (string?)questionElement.Element(ARTIST_ELEMENT) ?? String.Empty;
			var file_name = (string?)questionElement.Element(FILE_NAME_ELEMENT) ?? String.Empty;	// 相対パスをフルパスに直す作業が必要！
			...
			var memo = (string?)questionElement.Element(MEMO_ELEMENT) ?? String.Empty;
...
;			}
		...
		public static SweetQuestion GenerateFromMutus2(XElement songElement, string songsRoot, string category)
		{
			...
			question.Title = (string?)songElement.Element(TITLE_ELEMENT) ?? String.Empty;
			question.Artist = (string?)songElement.Element(ARTIST_ELEMENT) ?? String.Empty;
			var file_name = (string?)songElement.Element("filename") ?? String.Empty;	// 相対パスをフルパスに直す作業が必要！
			if (!Path.IsPathRooted(file_name))
			{
				file_name = Path.Combine(songsRoot, file_name);
				if (!Path.IsPathRooted(file_name))
